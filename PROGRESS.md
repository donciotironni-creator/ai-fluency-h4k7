...
- **2026-07-30** — **S7 (Plugins) — material-sursă gata, pagina NU** (AIF-2, rămâne `in-progress`). Scrise: `ghiduri/02-ghid-facilitare-S7.md` + `prezentare-S7.md` + `03-exercitii-S7.md`. **Rămas:** `s7.html` (încă ciotul de ~90 linii), cardul S7 din `index.html` (încă `coming-soon`). Verificat pe `code.claude.com/docs/en/plugins` + `/plugin-marketplaces` + `/discover-plugins` + `/settings`. **Abateri față de ciot:** „output styles" nu mai e director de componentă (componentele curente: `skills/`, `commands/` legacy, `agents/`, `hooks/hooks.json`, `.mcp.json`, `.lsp.json`, `monitors/`, `bin/`, `settings.json`); instalarea e în doi pași + `/reload-plugins`; skill-urile de plugin sunt obligatoriu namespaced `/plugin:skill`. **Atenție la formele de settings:** `extraKnownMarketplaces` și `enabledPlugins` sunt **obiecte**, nu liste (`"plugin@marketplace": true`) — verificat pe `~/.claude/settings.json` real, pentru că doc-ul rezumat le dădea ca array-uri. **Tot netratat din QA-ul de azi:** `glossary.js` linkuiește termeni în `.template-block`/`code` (s2, s3, s6, artefacte), `artefacte.html` are 17 nav pills în loc de 16, AIF-6 (Recuperare rapidă pe s3/s4/s5), `cheatsheet.html` fără conținut MCP (AIF-9), drift Bitbucket vs GitHub (`git.html` vs `03-exercitii-S4.md`).

- **2026-07-30** — **S6 (MCP) COMPLET, comis** `f87d7c0` (AIF-1, închis `done`): `ghiduri/02-ghid-facilitare-S6.md` + `prezentare-S6.md` + `03-exercitii-S6.md` + `s6.html` rescris din ciot (panel facilitator 6 segmente + panel echipă cu prerechizite și 6 exerciții + 5 self-check + „Recuperare rapidă"), plus `index.html` card S6 `coming-soon`→`done`. Verificat pe docul oficial `code.claude.com/docs/en/mcp` + `/permissions` + `/settings` înainte de redactare. **Abateri față de ciot:** SSE e deprecat (remote → `http`/`streamable-http`); „SSE = pentru echipă" e greșit — partajarea ține de scope, nu de transport; scope-urile sunt `local`/`project`/`user` (nu `project`/`global`); adăugate `ws`, tool search default-on, limite de output MCP, permisiuni `mcp__server__tool`, resources/prompts. **Sesiuni complete: S0–S6.** Următoarea: S7 (Plugins).
